...
class seed_average_onerun():
    ...
    def traincurves_and_iprs(self,non_grokked_object,remove_wdloss=False):
        ...
        #losses
        if remove_wdloss:
            yvalues_self=self.train_losses-(non_grokked_object.trainargs.weightdecay)*np.array(non_grokked_object.weight_norms)#not square-rooted
        else:
        ...
        #     )
        if remove_wdloss:
            yvalues=non_grokked_object.train_losses-(non_grokked_object.trainargs.weightdecay)*np.array(non_grokked_object.weight_norms)#not square-rooted
        else:
            yvalues=non_grokked_object.train_losses
        fig.add_trace(go.Scatter(x=list(range(len(non_grokked_object.test_losses))),y=yvalues,mode='lines',line=dict(color='blue',dash='dash'),showlegend=False,name=r'$\text{Learning train}$'),row=1,col=4)
