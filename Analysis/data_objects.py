...
class seed_average_onerun():
    ...
    def weights_histogram_epochs2(self,non_grokked_object):
        print('this one?')
        epochs=self.model_epochs()+[self.model_epochs()[-1]]#self.model_epochs()[::5]#[self.model_epochs()[0]]+[self.model_epochs()[5]]+self.model_epochs()[50:250][0::10]+[self.model_epochs()[-1]]
    ...
    def traincurves_and_iprs(self,non_grokked_object,remove_wdloss=False):
        ...
        #losses
        if remove_wdloss:
            removedloss_self=self.train_losses-(self.trainargs.weight_decay)*np.array(self.norms)#not square-rooted
        else:
        ...
        #     )
        if remove_wdloss:
            removedloss_ng=non_grokked_object.train_losses-(non_grokked_object.trainargs.weight_decay)*np.array(non_grokked_object.norms)#not square-rooted
        else:
            removedloss_ng=non_grokked_object.train_losses
        fig.add_trace(go.Scatter(x=list(range(len(non_grokked_object.train_losses))),y=removedloss_ng,mode='lines',line=dict(color='blue',dash='dash'),showlegend=False,name=r'$\text{Learning train}$'),row=1,col=4)
