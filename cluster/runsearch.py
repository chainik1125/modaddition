...
def files_summary(directory,variables):
    ...
    for dirpath,_,filenames in os.walk(directory):
        for filename in filenames:
            print(filename)
            if 'grok_True' in  dirpath or 'grok_True' in filename:  # or whatever file extension your files have
    ...
    for dirpath,_,filenames in os.walk(directory):
        for filename in filenames:
            if 'grok_False' in  dirpath or 'grok_False' in filename:  # or whatever file extension your files have
                file_path = os.path.join(dirpath, filename)
                # Load the object
                file_object = torch.load(file_path,map_location='cpu')
                # Access the attributes and add them to the table
...
variables=['lr','hiddenlayers','weight_decay','weight_multiplier','grok','epochs']
